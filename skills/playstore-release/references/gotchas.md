...
**403 on validate/commit for listing or contact-detail changes while releases work.** The app is still a _draft_ — it has never passed review — and editing a draft app's store presence needs its own permission: **Create, edit and delete draft apps** (Users and permissions → the service account → _Account permissions_ → "Taslak uygulamalar"). "Manage store presence" alone is not enough until the first review. Tracks keep working because "Release to testing tracks" includes uploading to draft apps. Seen 2026-09-21: `edits:validate` 403 with details/listings in the edit, OK with only a track change; ticking the box fixed it within seconds.
